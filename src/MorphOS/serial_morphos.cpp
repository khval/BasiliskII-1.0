...
static const uint32 MSG_QUERY = 'qery';			// Query port status, return status in control_io
static const uint32 MSG_SET_PARAMS = 'setp';		// Set serial parameters (parameters in control_io)
static const uint32 MSG_SET_PAR_PARAMS = 'pstp';	// Set parallel parameters (parameters in control_io)
static const uint32 MSG_KILL_IO = 'kill';			// Kill pending I/O requests
static const uint32 MSG_BREAK = 'brek';			// Send break
static const uint32 MSG_RESET = 'rset';			// Reset channel
static const uint32 MSG_PRIME_IN = 'prin';			// Data input
static const uint32 MSG_PRIME_OUT = 'pout';		// Data output
...
private:
	...
	static void serial_func(ASERDPort *);
...
int16 ASERDPort::open(uint16 config)
{
	...
	startup.mn_Node.ln_Type	= NT_MESSAGE;
	startup.mn_ReplyPort		= StartupMsgPort;
	startup.mn_Length			= sizeof(struct Message);
...
void ASERDPort::serial_func(ASERDPort *proc_arg)
{
	...
	if (io_port == NULL || control_port == NULL)
		goto quit;
	...
	if (OpenDevice(dev_name, dev_unit, (struct IORequest *)read_io, 0) || read_io->IOSer.io_Device == NULL)
		goto quit;
	opened = true;

	// Copy IORequests
	CopyMem(read_io, write_io, sizeof(struct IOExtSer));
	CopyMem(read_io, control_io, sizeof(struct IOExtSer));
