...
int stub_audio_callback_PPC(ULONG *regarray);
static  ULONG audio_callback(struct Hook *hook /*a0*/, struct AHISoundMessage *msg /*a1*/, struct AHIAudioCtrl *ahi_ctrl /*a2*/);
...
//	set_audio_status_format();
	...
	sf_hook.h_Entry = (HOOKFUNC) stub_audio_callback_PPC;
...
int stub_audio_callback_PPC(ULONG *regarray)
{
	struct Hook *hook = (struct Hook *) regarray[REG68K_A0/4];				/*a0*/
	struct AHISoundMessage *msg = (struct AHISoundMessage *) regarray[REG68K_A1/4];		/*a1*/
	struct AHIAudioCtrl *ahi_ctrl = (struct AHIAudioCtrl *) regarray[REG68K_A2/4];			/*a2*/

	return regarray[REG68K_D0/4] = audio_callback( hook , msg , ahi_ctrl );
}
