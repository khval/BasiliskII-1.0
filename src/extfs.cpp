...
#ifdef __AMIGAOS4__
	extern void stccpy( char * des , char * src, int len );
#endif
...
//		printf("void get_path_for_fsitem(FSItem *p) line %d\n",__LINE__);
		full_path[0] = 0;
	} else if (p->id == ROOT_ID) {
//		printf("void get_path_for_fsitem(FSItem *p) line %d\n",__LINE__);
//		printf("full_path 0x%08x\n",full_path);
//		printf("RootPath 0x%08x\n",RootPath);
		stccpy(full_path,(char *) RootPath, MAX_PATH_LENGTH-1);
...
void ExtFSInit(void)
{
	...
	stccpy(p->name, (char *) GetString(STR_EXTFS_VOLUME_NAME), 32);

	// Find path for root
	if ((RootPath = (char *) PrefsFindString("extfs")) != NULL) {
